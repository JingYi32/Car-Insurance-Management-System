...
namespace IOOP_Assignment___Car_Insurance_Management_System
{
    public partial class Updating_Insurance : Form
    {
        ...
        private void btnSubmit_UI_Click(object sender, EventArgs e)
        {
            ...
            cmd.CommandText = "UPDATE insurance SET Ins_status ='Claimed' WHERE (select MAX(ins_enddate) AS [Insurance End Date] FROM Insurance WHERE id = '"+Save.insuranceid+"') AND id='"+Save.insuranceid+"'";
            cmd.Connection = con;
            cmd.ExecuteNonQuery();
        }
